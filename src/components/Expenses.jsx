...
const Expenses = ({ expenses, accounts = [], onAdd, onEdit, onDelete, onMarkPaid, onRefresh, darkMode, theme }) => {
  ...
  const emptyForm = { name: '', amount: '', type: 'monthly', dueDate: '', status: 'غير مدفوع', accountId: '', notes: '', code: '' };
  const [formData, setFormData] = useState(emptyForm);
  const [errors, setErrors] = useState({});

  const filteredExpenses = expenses.filter(expense => {
    const matchSearch = expense.name?.toLowerCase().includes(searchTerm.toLowerCase()) || expense.code?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const openAddModal = () => { setFormData({ ...emptyForm, code: generateCode('expenses') }); setErrors({}); setShowAddModal(true); };
  const openEditModal = (expense) => {
    setSelectedExpense(expense);
    setFormData({ name: expense.name || '', amount: expense.amount || '', type: expense.type || 'monthly', dueDate: expense.dueDate || '', status: expense.status || 'غير مدفوع', accountId: expense.accountId || '', notes: expense.notes || '', code: expense.code || '' });
    setErrors({}); setShowEditModal(true);
  };
  const openDeleteModal = (expense) => { setSelectedExpense(expense); setShowDeleteModal(true); };

  const handleAdd = async () => {
    if (!validateForm()) return;
    setLoading(true);
    try { await onAdd({ ...formData, amount: parseFloat(formData.amount) }); setShowAddModal(false); setFormData(emptyForm); } catch (e) { console.error(e); }
    setLoading(false);
  };
  ...
  const handleEdit = async () => {
    if (!validateForm()) return;
    setLoading(true);
    try { await onEdit({ ...selectedExpense, ...formData, amount: parseFloat(formData.amount) }); setShowEditModal(false); } catch (e) { console.error(e); }
    setLoading(false);
  };

  const handleDelete = async () => {
    setLoading(true);
    try { await onDelete(selectedExpense.id); setShowDeleteModal(false); } catch (e) { console.error(e); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ padding: '24px 0', paddingBottom: 100 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h2 style={{ fontSize: 24, fontWeight: 700, color: t.text.primary, margin: 0, display: 'flex', alignItems: 'center', gap: 10 }}><Receipt size={28} />المصروفات</h2>
          <p style={{ fontSize: 14, color: t.text.muted, marginTop: 4 }}>إدارة المصروفات الشهرية والسنوية</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onRefresh} style={{ width: 40, height: 40, borderRadius: 10, border: `1px solid ${t.border.primary}`, background: t.bg.secondary, color: t.text.muted, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><RefreshCw size={18} /></button>
          <button onClick={openAddModal} style={addButtonStyle}><Plus size={18} />إضافة مصروف</button>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[{ label: 'إجمالي الشهري', value: `${formatNumber(monthlyTotal)} ريال`, color: t.colors[colorKeys[0]]?.main }, { label: 'إجمالي السنوي', value: `${formatNumber(yearlyTotal)} ريال`, color: t.colors[colorKeys[1]]?.main }, { label: 'غير مدفوع', value: unpaidCount, color: t.status.danger.text }].map((stat, i) => (
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center', background: t.bg.secondary, padding: 12, borderRadius: 12, border: `1px solid ${t.border.primary}` }}>
        ...
        <div style={{ position: 'relative' }}>
          <select value={filterType} onChange={(e) => setFilterType(e.target.value)} style={filterSelectStyle}><option value="all">كل الأنواع</option><option value="monthly">شهري</option><option value="yearly">سنوي</option></select>
          <ChevronDown size={16} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: t.text.muted, pointerEvents: 'none' }} />
        </div>
        <div style={{ position: 'relative' }}>
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} style={filterSelectStyle}><option value="all">كل الحالات</option><option value="مدفوع">مدفوع</option><option value="غير مدفوع">غير مدفوع</option></select>
          <ChevronDown size={16} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: t.text.muted, pointerEvents: 'none' }} />
        </div>
      </div>
      ...
      {filteredExpenses.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filteredExpenses.map((expense, index) => {
            ...
            return (
              <div key={expense.id} style={{ width: '100%', background: t.bg.secondary, borderRadius: 12, border: `1px solid ${t.border.primary}`, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: color.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Receipt size={20} color="#fff" /></div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: color.main, fontFamily: 'monospace', background: `${color.main}15`, padding: '2px 6px', borderRadius: 4 }}>{expense.code || 'E-0000'}</span>
                    <h3 style={{ fontSize: 14, fontWeight: 600, color: t.text.primary, margin: 0 }}>{expense.name}</h3>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 5, background: t.status.info.bg, color: t.status.info.text }}>{expense.type === 'monthly' ? 'شهري' : 'سنوي'}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: color.main }}>{formatNumber(expense.amount)} ريال</span>
                    <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 5, background: paidStyle.bg, color: paidStyle.text, border: `1px solid ${paidStyle.border}` }}>{expense.status}</span>
                    <span style={{ fontSize: 11, color: t.text.muted, display: 'flex', alignItems: 'center', gap: 3 }}><Calendar size={12} />{expense.dueDate}</span>
                    {!isPaid && days !== null && <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 5, background: daysStyle.bg, color: daysStyle.text, display: 'flex', alignItems: 'center', gap: 3 }}><AlertTriangle size={11} />{days < 0 ? `متأخر ${Math.abs(days)} يوم` : `متبقي ${days} يوم`}</span>}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                  {!isPaid && <button onClick={() => onMarkPaid(expense.id)} title="تحديد كمدفوع" style={{ width: 30, height: 30, borderRadius: 6, border: 'none', background: t.status.success.bg, color: t.status.success.text, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><CheckCircle size={14} /></button>}
                  <button onClick={() => openEditModal(expense)} style={{ width: 30, height: 30, borderRadius: 6, border: 'none', background: t.bg.tertiary, color: t.text.muted, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Edit size={14} /></button>
                  <button onClick={() => openDeleteModal(expense)} style={{ width: 30, height: 30, borderRadius: 6, border: 'none', background: t.status.danger.bg, color: t.status.danger.text, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Trash2 size={14} /></button>
                </div>
              </div>
            );
          })}
      ...
      <Modal show={showAddModal} onClose={() => setShowAddModal(false)} title="إضافة مصروف جديد" onSubmit={handleAdd} submitText="إضافة" loading={loading} theme={t}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div><label style={labelStyle}>النوع</label><select value={formData.type} onChange={(e) => updateForm('type', e.target.value)} style={inputStyle}><option value="monthly">شهري</option><option value="yearly">سنوي</option></select></div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div><label style={labelStyle}>تاريخ الاستحقاق *</label><input type="date" value={formData.dueDate} onChange={(e) => updateForm('dueDate', e.target.value)} style={{...inputStyle, borderColor: errors.dueDate ? t.status.danger.text : t.border.primary}} /></div>
            <div><label style={labelStyle}>الحالة</label><select value={formData.status} onChange={(e) => updateForm('status', e.target.value)} style={inputStyle}><option value="غير مدفوع">غير مدفوع</option><option value="مدفوع">مدفوع</option></select></div>
          </div>
          <div><label style={labelStyle}>الحساب</label><select value={formData.accountId} onChange={(e) => updateForm('accountId', e.target.value)} style={inputStyle}><option value="">اختر الحساب</option>{accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}</select></div>
          <div><label style={labelStyle}>ملاحظات</label><textarea value={formData.notes} onChange={(e) => updateForm('notes', e.target.value)} style={{...inputStyle, minHeight: 80, resize: 'vertical'}} placeholder="ملاحظات إضافية..." /></div>
        </div>
      </Modal>
      ...
      <Modal show={showEditModal} onClose={() => setShowEditModal(false)} title="تعديل المصروف" onSubmit={handleEdit} submitText="حفظ التعديلات" loading={loading} theme={t}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div><label style={labelStyle}>الحساب</label><select value={formData.accountId} onChange={(e) => updateForm('accountId', e.target.value)} style={inputStyle}><option value="">اختر الحساب</option>{accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}</select></div>
          <div><label style={labelStyle}>ملاحظات</label><textarea value={formData.notes} onChange={(e) => updateForm('notes', e.target.value)} style={{...inputStyle, minHeight: 80, resize: 'vertical'}} /></div>
        </div>
      </Modal>
      ...
      <Modal show={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="حذف المصروف" onSubmit={handleDelete} submitText="حذف" danger loading={loading} theme={t}>
        <div style={{ textAlign: 'center', padding: 20 }}>
          ...
          <p style={{ fontSize: 18, fontWeight: 700, color: t.status.danger.text }}>{selectedExpense?.code} - {selectedExpense?.name}</p>
